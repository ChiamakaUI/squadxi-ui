...
import {
  Bot, Zap, Shield, Trophy, ChevronRight, Check,
  Loader2,
} from "lucide-react";
...
export default function AgentPage() {
  const { data: agentStatus, isLoading, isError, error, refetch } = useAgentStatus();
  const notConfigured =
    isError &&
      ...
      <div className="space-y-6 max-w-2xl">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">
            Your agent watches open contests, builds squads, and enters on your behalf.
          </p>
        </div>
...
const DEFAULT_RULES = [
  ...
  },
  {
    ...
    description: "Balanced squad selection — not the highest ceiling, not the safest floor.",
...
function AgentSetupWizard() {
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      await updateSettings.mutateAsync({ totalDeposited: depositAmount, depositTx: tx });
      setStep(3);
    } catch (e: any) {
        ...
        >
          <div className="space-y-5">
            ...
            <SummaryBox>
              ...
              <span className="text-white font-bold">{formatUsdc(maxSpend * maxContests)}</span>
            </SummaryBox>
      ...
      {step === 2 && (
        ...
        >
          <div className="space-y-5">
            ...
            <SummaryBox>
              Depositing{" "}
              <span className="text-white font-bold">{formatUsdc(depositAmount)}</span>{" "}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <p className={cn("text-sm font-medium", on ? "text-white" : "text-zinc-400")}>
                        {r.label}
                      </p>
          ...
          <div key={step.id} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center gap-1.5">
              <div className={cn(
                ...
                : active ? "bg-emerald-500/20 border border-emerald-500/50 text-emerald-400"
              ...
              <span className={cn(
                ...
                active ? "text-emerald-400" : done ? "text-zinc-400" : "text-zinc-700"
              )}>
                {step.label}
              </span>
            </div>
...
function SetupCard({ title, description, children }: {
  ...
  return (
    <div className="rounded-2xl border border-white/8 bg-white/[0.02] overflow-hidden">
      ...
      <div className="p-6 sm:p-8 space-y-6">
        <div>
          <h2 className="font-display text-xl font-black uppercase tracking-tight text-white">{title}</h2>
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-baseline">
        ...
        <span className="font-display text-xl font-black text-emerald-400">{format(value)}</span>
      </div>
      <input
...
function SummaryRow({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="flex justify-between items-center px-4 py-3">
      ...
      <span className={cn("text-sm font-bold", accent ? "text-emerald-400" : "text-white")}>{value}</span>
    </div>
  );
}
